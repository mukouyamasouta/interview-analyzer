...
// ── Safe JSON parser (handles truncated/malformed responses) ──────────────
function safeJsonParse(text) {
  ...
  // 2. Try to repair truncated JSON by closing open structures
  try {
    let repaired = s;
    repaired = repaired.replace(/,\s*"[^"]*"\s*:\s*[^,}\]]*$/, ""); // incomplete key-value
...
function parseTranscriptText(rawText) {
  ...
  for (const line of lines) {
    const m = line.match(/^\[(面接官|志望者)(?:\s+(\d{1,2}:\d{2}))?\]\s*(.+)/);
    if (m) {
      entries.push({
        speaker: m[1] === '面接官' ? 'interviewer' : 'candidate',
        time: m[2] || '',
        text: m[3].trim(),
      });
...
const QA_PROMPT = (transcript, company, type) => `あなたは面接記録の分析専門家です。以下の面接文字起こしから、面接官が発した質問・問いかけを【一つも漏らさず】すべて抽出してください。
...
【作業手順】
まず文字起こし全体を通読し、[面接官]の発言を全てリストアップする。
次に、各発言について以下の基準に照らして「質問かどうか」を判定する。
疑わしい場合は必ず質問として含める（除外方向に判断しない）。

【質問と判断する基準（1つでも該当すれば質問として抽出する）】
1. 文末に「？」「ですか」「ますか」「でしょうか」「いかがですか」「どうですか」が付いている
2. 「教えてください」「お聞かせください」「お伺いしたい」「聞かせてほしい」など情報を求める表現がある
3. 「どのように」「なぜ」「どんな」「何が」「いつ」「どこで」「どれくらい」「どういった」など疑問詞を含む
4. 志望者の発言に対して面接官が返した発言で、掘り下げ・確認・言い換えを求めている（例：「具体的には？」「それはどういうことですか」「もう少し詳しく」「なるほど、それで？」）
5. 「〜ということですか」「〜で合っていますか」「〜という理解でよいですか」など確認・リフレーズの形
6. 「〜についてはどう思いますか」「〜の点はどうでしょう」など間接的・婉曲な問いかけ
7. 短い相槌・促しの後に続く問いかけ（例：「なるほど。では〜は？」「そうですか。〜についてはいかがですか」）

【必須ルール】
- 掘り下げ質問・フォローアップ質問も独立したエントリーとして必ず出力する
- 面接官の一発言に複数の質問が含まれる場合は、質問ごとに分割して別エントリーにする
- 志望者の回答は、その質問への回答として話した内容を原文に忠実・完全に記録する（要約・省略しない）
- 回答が複数の発言にまたがる場合はすべて結合する
- 抽出漏れを防ぐため、出力前に「[面接官]の発言数」と「抽出したQ&A数」が近いことを確認する
- 除外してよいのは「よろしくお願いします」「お疲れ様でした」などの純粋な挨拶・締めの発言のみ
  ...
  "qaList": [
    {
      ...
      "question": "面接官の質問・問いかけ(原文に近い形で、省略しない)",
        ...
        {mode==="upload"&&(
          <div
            ...
            style={{
              ...
              border:`2px dashed ${file?C.koke:dragOver?C.shu:C.tan2}`,
            ...
            <div style={{ width:"64px",height:"64px",display:"flex",alignItems:"center",justifyContent:"center",background:file?C.koke:dragOver?C.shu:C.tan,borderRadius:"50%",marginBottom:"20px" }}>
              {file ? <CheckCircle2 size={28} strokeWidth={1.5} style={{ color:C.cream }}/>
                    : dragOver ? <Upload size={28} strokeWidth={1.2} style={{ color:C.cream }}/>
                    : <Upload size={28} strokeWidth={1.2} style={{ color:C.ink60 }}/>}
            </div>
            {file
              ? <>
              ...
              : dragOver
              ? <>
                  <p style={{ fontFamily:F.min,fontSize:"1.1rem",color:C.shu,marginBottom:"6px",marginTop:0 }}>ここにドロップ</p>
                  <p style={{ fontSize:"12px",color:C.shu,margin:0,opacity:0.8 }}>ファイルを離してください</p>
                </>
              : <>
...
export default function App() {
  ...
  const [busy, setBusy] = useState(null); // null | "upload" | "transcribe" | "qa" | "feedback"
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // ページリロード後に前回のセッションを復元
      try {
        const cur = await window.storage.get("session:current");
        if (cur?.value) {
          const s = JSON.parse(cur.value);
          setSession(s);
          setSaved(false);
          // 最後にみていたタブを復元（フィードバックがあれば分析タブ）
          if (s.feedback) setTab("feedback");
          else if (s.qaList?.length) setTab("qa");
          else if (s.transcript?.length) setTab("transcript");
        }
      } catch {}
  ...
  // 現在のセッションを session:current として自動保存
  const persistCurrent = async (s) => {
    try { await window.storage.set("session:current", JSON.stringify(s)); } catch {}
  };
  ...
  const runAnalysis = (args) => {
    const doRun = async () => {
      ...
      if (mode === "audio") {
        const INLINE_LIMIT = 19 * 1024 * 1024;
        if (file.size > INLINE_LIMIT) {
          // Large file: show upload stage first
      ...
      // Step 1 完了: transcript + 自動保存
      const partialSession = { sessionId, company, iType, date, transcript, qaList: null, feedback: null };
      setSession(partialSession);
      await persistCurrent(partialSession);
      setTab("transcript");
      ...
      const qaSession = { sessionId, company, iType, date, transcript, qaList, feedback: null };
      setSession(qaSession);
      await persistCurrent(qaSession);
      setTab("qa");
  ...
  // 現在のセッションをリセット・新規分析に準備
  const clearCurrentSession = async () => {
    setSession(null);
    setSaved(false);
    setTab("input");
    try { await window.storage.delete("session:current"); } catch {}
  };
          ...
          <div style={{ display:"flex",alignItems:"center",gap:"12px" }}>
            {session && (
              <button onClick={clearCurrentSession}
                title="現在の分析をリセットして新規分析を開始"
                style={{ display:"flex",alignItems:"center",gap:"6px",padding:"6px 12px",fontSize:"11px",letterSpacing:"0.1em",
                  border:`1px solid ${C.tan2}`,background:"transparent",color:C.ink60,cursor:"pointer",fontFamily:F.sans,transition:"all 0.2s" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=C.shu;e.currentTarget.style.color=C.shu;}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=C.tan2;e.currentTarget.style.color=C.ink60;}}>
                <RefreshCw size={12} strokeWidth={1.5}/>新規分析
              </button>
            )}
