...
// ── Default API Key ───────────────────────────────────────────────────────
const DEFAULT_API_KEY = "AIzaSyCvUICumci2996nOitqxXzvGxyc7soFVAU";
...
const QA_PROMPT = (transcript, company, type) => `あなたは面接記録の分析専門家です。以下の面接文字起こしから、面接官が発した【すべての質問・問いかけ】を一つも漏らさず抽出してください。

【面接情報】
- 会社名: ${company || "(未指定)"}
- 面接種別: ${type}

【質問と判断する基準（以下のいずれかに該当すれば必ず質問として抽出する）】
1. 文末に「？」「ですか」「ますか」「でしょうか」「いかがですか」が付いている
2. 「〜についてお聞きしたい」「〜をお伺いしたい」「〜を教えてください」「〜をお聞かせください」など、情報を求める表現がある
3. 「〜はどうですか」「〜はどのように」「〜はなぜ」「〜はいつ」「〜は何」など疑問詞・疑問表現を含む
4. 志望者が何かを話した直後に面接官が返した発言で、掘り下げや確認を求めている（例：「具体的には？」「もう少し詳しく」「それはどういう意味ですか」）
5. 「〜ということですか？」「〜で合っていますか？」など確認・言い換えの形式
6. 「〜について聞いてもいいですか」「〜の点はどうでしょう」など間接的な問いかけ
...
- 面接の最初の「よろしくお願いします」「本日はよろしく」などの挨拶のみの発言は除外する
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        const OLD_KEYS = ["AIzaSyAglc7JmXn5w4OWT3dpGoLWsVwXy6dRx18","AIzaSyCnfO2-geRJTfOMXibAjCJ3aSD58DD9Yoo"];
